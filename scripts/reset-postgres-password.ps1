    ...
    try {
        if ($DryRun) {
            ...
            if ($LASTEXITCODE -eq 0) {
                ...
                Write-ResetLog "⚠️ Backup failed, continuing without backup..." "Yellow" "⚠️"
                return $null
            }
        }
    } catch {
        Write-ResetLog "⚠️ Backup failed: $_" "Yellow" "⚠️"
        return $null
    }
}
    ...
    try {
        ...
        if ($BackupFile -and (Test-Path $BackupFile) -and $PreserveData) {
            ...
            if ($LASTEXITCODE -eq 0) {
                ...
                Write-ResetLog "⚠️ Data restoration had issues" "Yellow" "⚠️"
            }
        }
...
if (Test-Path $envPath) {
    ...
    if (Test-PostgreSQLConnection -Password $currentPassword) {
        ...
        Write-ResetLog "⚠️ Current connection failed - proceeding with reset" "Yellow" "⚠️"
    }
} else {
    Write-ResetLog "⚠️ .env file not found - proceeding with reset" "Yellow" "⚠️"
}
...
if (!$DryRun) {
    ...
    if ($backupFile) {
        Write-ResetLog "4. Remove backup file when no longer needed: $backupFile" "White" "🗑️"
    }
}